...
export async function GET(request, { params }) {
  try {
    ...
    const emailIndex = pathParts.findIndex(part => part === 'send-gmail-verification-email');
    ...
    try {
      const verificationLink = `${process.env.SERVER_URL || 'http://localhost:3000'}/telstra/email-verification?email=${encodeURIComponent(emailAddress)}`;
      ...
      console.log(`Verification email sent successfully to ${emailAddress}`);
    ...
    console.error('Error in send-gmail-verification-email:', error);
    return NextResponse.json(
